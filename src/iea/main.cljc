(ns iea.main
  "Kotodama WASM entrypoint for the Iea clean-room actor (L4) — Clojure port.
  ...
  facts (`iea.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"iea.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iea-compat" :tier tier :entities entities} 200])
